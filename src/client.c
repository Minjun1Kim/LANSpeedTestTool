...
void run_udp_upload_test(char *address, int port, int duration) {
    ...
    while (total_elapsed_time < 0.1) {
        ...
        long bytes_sent_i = sendto(sock, data, DATA_SIZE, 0, (struct sockaddr*)&server_addr, addr_len);
        if (bytes_sent_i < 0) {
...
void run_udp_download_test(char *address, int port, int duration) {
    ...
    while (total_elapsed_time < duration) {
        clock_gettime(CLOCK_MONOTONIC, &start_time);
        long bytes_recieved_i = recvfrom(sock, buffer, DATA_SIZE, 0, (struct sockaddr*)&server_addr, &addr_len);
